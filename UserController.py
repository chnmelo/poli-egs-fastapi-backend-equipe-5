...
class UserController:
    ...
    def login(self, email: str, password: str):
        try:
            ...
            user_record = admin_auth.get_user(user['localId'])

            # Se o e-mail não foi verificado, impede o login
            if not user_record.email_verified:
                # Reenviar o e-mail de verificação
                self.auth.send_email_verification(user['idToken'])
                raise HTTPException(
                    status_code=401,
                    detail="Por favor, verifique seu e-mail antes de fazer o login. Um novo e-mail de verificação foi enviado."
                )
